...
//#include "examples/structural/example_6/stiffened_plate_thermally_stressed_piston_theory_flutter_optimization.h"
...
//        _thz_station_vals(nullptr)
    {
        ...
        libMesh::out
                ...
                << "  _input.in should be provided in the working directory with"
        ...
        _n_load_steps = _input("n_load_steps", " ", 20);
...
//            _flutter_solver  = new MAST::TimeDomainFlutterSolver;
    ...
    void _init_mesh() {
        ...
        // number of stations
        _n_dv_stations_x = _input("n_stations", " ", 4);
...
//        constrained_vars[3] = 5;  // tz
    ...
    void _init_piston_vals() {
        ...
        _velocity_f = new MAST::ConstantFieldFunction("V", *velocity);
        _mach_f = new MAST::ConstantFieldFunction("mach", *mach);
        _rho_air_f = new MAST::ConstantFieldFunction("rho", *rho_air);
        _gamma_air_f = new MAST::ConstantFieldFunction("gamma", *gamma_air);
        ...
        _piston_bc = new MAST::PistonTheoryBoundaryCondition(1,     // order
                                                             vel);  // vel vector
        _piston_bc->add(*velocity_f);
        _piston_bc->add(*mach_f);
        _piston_bc->add(*rho_air_f);
        _piston_bc->add(*gamma_air_f);
    ...
    void _init_loads() {
        ...
        _p_cav = new MAST::Parameter("p_cav", _input("p_cav", "", 0.));
        ...
        _temp       = new MAST::Parameter("temperature", _input("temp", "", 60.));
    ...
    void _init_outputs(){
        ...
        for (; e_it != e_end; e_it++) {
            ...
            output->set_aggregation_coefficients(_p_val,_vm_rho,_stress_limit);
...
//                for (int i=0; i < _outputs.size(); i++) {
                ...
                stress_elem.set_aggregation_coefficients(_p_val,_vm_rho,_stress_limit);
...
//                }
            ...
            if (if_sens) {
                ...
                //MAST::FunctionBase& params(nullptr);
                //params.resize(1);
...
//
//                    _sys->sensitivity_solve(*_nonlinear_elem_ops,*_nonlinear_assembly,*params,false);
//
//
...
//                    params[0] = _problem_parameters[i]->ptr();
    ...
    public:
        StiffenedPlateSteadySolverInterface(StiffenedPlateThermallyStressedPistonTheorySizingOptimization& obj,
                ...
                inc (0) {
            _obj._sys->add_vector("base_solution"); // add vector in sys call it bas_solution will store disps
